...
list *newList(item e){
  ...
  node* senNode = malloc(sizeof(node));
  senNode -> x = e;
  l -> none = senNode;
  l -> current = senNode;
  senNode -> back = l -> none;
...
void freeList(list *xs){
    node *currrent = xs->none->next;
    while(currrent!=(xs->none)){
...
void first(list *xs){
  xs->current = xs->none->next; //can use node*p = xs -> current etc for more complicated ones
}
...
void insertAfter(list *xs, item x){
  ...
  newNode->x = x;
  node*p = xs->current;
  node*q = xs->current->next; //original current + next
...
void insertBefore(list *xs, item x){
  ...
  newNode->x = x;
  node*p = xs->current;
  node*q = xs->current->back; //original current + next
